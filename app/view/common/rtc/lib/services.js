...
if(IS_BROWSER) {
  socket = io();
    ...
    socket = socketIOClient('https://creek.xin:13229', {
        transports: ['websocket']
    });
    socket.on('connect', () => {
        console.log('connected!');
    });
    // socket = io.connect('https://creek.xin:13229', {transports: ['websocket']});
...
function getLocalStream(isFront, callback) {
    WebRTC.MediaStreamTrack.getSources(sourceInfos => {
        // sourceInfos 摄像头和音频信息
        console.log('sourceInfos', sourceInfos);
        
        let videoSourceId;    // 摄像头ID
        ...
        WebRTC.getUserMedia({
            ...
            video: {
                ...
                /*
                    user: 视频源面向用户; 这包括例如智能电话上的前置摄像头。
                    environment: 视频源远离用户，从而查看其环境。这是智能手机上的后置摄像头。
                */
...
function join(roomId, name, callbacks) {
    ...
    socket.emit('join', {roomId, name}, function(result){
        console.log('Joins', friends, result, roomId, name);        
        friends = result;
